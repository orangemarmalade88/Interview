...
public class Solution {
	...
	static class SetAssociativeCacheRunner {
		public static void parseInput(InputStream inputStream)
				throws IOException {
			...
			while (!isNullOrEmpty(line = reader.readLine())) {
				...
				if (lineCount == 1) {
					...
					// All remaining lines invoke instance methods on the
					// SetAssociativeCache
					Object retValue = SetAssociativeCacheFactory
							.InvokeCacheMethod(line, cache);
	...
	private static SetAssociativeCache<String, String> createCache(
		...
		return SetAssociativeCacheFactory.CreateStringCache(setCount, setSize,
				replacementAlgoName.value);
	}
	...
	/**
	 * NOTE: You are free to modify anything below, except for class names and
	 * generic interface. Other public interface changes may require updating
	 * one or more of the helper classes above for test cases to run and pass.
	 * <p>
	 * A Set-Associative Cache data structure with fixed capacity.
	 * <p>
	 * - Data is structured into setCount # of setSize-sized sets. - Every
	 * possible key is associated with exactly one set via a hashing algorithm -
	 * If more items are added to a set than it has capacity for (i.e. > setSize
	 * items), a replacement victim is chosen from that set using an LRU
	 * algorithm.
	 * <p>
	 * NOTE: Part of the exercise is to allow for different kinds of replacement
	 * algorithms...
	 */
	public static class SetAssociativeCache<TKey, TValue> {
		...
		public SetAssociativeCache(int setCount, int setSize,
			...
			this.replacementAlgoName = replacementAlgoName;
			Solution s = new Solution();
			ReplacementAlgoFactory factory = s.new ReplacementAlgoFactory();
			this.replacementAlgo = factory
					.createReplacementAlgo(replacementAlgoName);
		...
		/**
		 * Adds the `key` to the cache with the associated value, or overwrites
		 * the existing key. If adding would exceed capacity, an existing key is
		 * chosen to replace using an LRU algorithm (NOTE: It is part of this
		 * exercise to allow for more replacement algos)
		 */
		...
		/**
		 * Returns `true` if the given `key` is present in the set; otherwise,
		 * `false`.
		 */
	...
	/**
	 * An internal data structure representing one set in a N-Way
		...
		public TValue get(TKey key) {
			...
			if (this.containsKey(key)) {
				...
				throw new RuntimeException(
						String.format("The key '%s' was not found", key));
			}
		...
		public void set(TKey key, TValue value) {
			...
			if (indexOfKey >= 0) {
				...
				if (this.Count == this.Capacity) {
					// Choose the Least-Recently-Used (LRU) item to replace,
					// which will be at the tail of the usage tracker
					// TODO: Factor this logic out to allow for custom
					// replacement algos
					TKey keyToReplace = (TKey) replacementAlgo
							.indexToReplace(UsageTracker);
	...
	/**
	 * An internal data structure representing a single item in an N-Way
	 * Set-Associative Cache
	 */
	...
	/**
	 * A common interface for replacement algos, which decide which item in a
	 * CacheSet to evict
	 */
	interface IReplacementAlgo<TKey> {
		TKey indexToReplace(LinkedList<TKey> UsageTracker);
	}

	class LRUReplacementAlgo<TKey> implements IReplacementAlgo<TKey> {
		@Override
		public TKey indexToReplace(LinkedList<TKey> UsageTracker) {
			return UsageTracker.getLast();
		}
	}

	class MRUReplacementAlgo<TKey> implements IReplacementAlgo<TKey> {
		@Override
		public TKey indexToReplace(LinkedList<TKey> UsageTracker) {
	...
	// ############################
	// NOTE: Your code in the classes below will not be evaluated as part of the
	// exericse.
	// They are just used by the stub code in the header to help run HackerRank
	// test cases.
	// You may need to make small modifications to these classes, depending on
	...
	public static class SetAssociativeCacheFactory {
		...
		/// NOTE: Modify only if you change the main interface of
		/// SetAssociativeCache
		public static Object InvokeCacheMethod(String inputLine,
				SetAssociativeCache<String, String> cacheInstance) {
			...
			// String[] callParams = Arrays.copyOfRange(callArgs, 1,
			// callArgs.length - 1); // TODO: This is unused
			...
			switch (methodName) {
				...
				// TODO: If you want to add and test other public methods to
				// SetAssociativeCache,
				// add them to the switch statement here... (this is not common)

				default:
					throw new RuntimeException(String
							.format("Unknown method name '{%s}'", methodName));
			}
		}
	}

	// TODO: Consider making use of this in the `SetAssociativeCacheFactory`
	// above to map replacement algo name
	...
	public class ReplacementAlgoFactory {
		IReplacementAlgo createReplacementAlgo(String replacementAlgoName) {
			switch (replacementAlgoName) {
				...
				default:
					// TODO: If you want to test other replacement algos, add
					// them to the switch statement here...
					throw new RuntimeException(
							String.format("Unknown replacement algo '%s'",
									replacementAlgoName));
			}
		}
	}
